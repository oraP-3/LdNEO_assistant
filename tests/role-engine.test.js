...
test('9牌時の切り牌候補表示はposition analyzerを利用する',()=>{
  const appSource=readFileSync(new URL('../assets/js/app.js',import.meta.url),'utf8');
  assert.match(appSource,/import \{ analyzeDiscardCandidates \} from '\.\/position-analyzer\.js';/);
  assert.match(appSource,/analyzeDiscardCandidates\(state\.selectedHand,calcContext\(\)\)/);
  assert.doesNotMatch(appSource,/checkMachi\(hand8,calcContext\(\),\[discard\.id\]\)/);
});
